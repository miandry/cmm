...
class AbstractEntityParser
{
        ...
        if ($bool) {
            ...
            foreach ($medias as $key => $media) {
                ...
                if($media->bundle() == 'image'){
                    ...
                    $result[] = [
                        'document' => $this->ers_451e8e6d7b53f8a06e3f8517cf02b856_image_file($media, 'field_media_document'),
                        'mid'=> $media->id(),
                        'name'=> $media->label(),
                        'object' => $media,
                        '#object' => $media
                    ...
                    $result[] = [
                        'video' => $this->ers_451e8e6d7b53f8a06e3f8517cf02b856_image_file($media, 'field_media_video_file'),
                        'mid'=> $media->id(),
                        'name'=> $media->label(),
                        'object' => $media,
                        '#object' => $media
    ...
    protected function entity_parser_load_default($entity, $fields, $options)
    {
        $item = [];
        $that = $this ;
        if(isset($options['#hook_class'])){
            $class = $options['#hook_class'];
            $that = new $class();
        }
        ...
        foreach ($fields as $key => $field) {
            ...
            if ($entity->hasField($field) && !$entity->get($field)->isEmpty()) {
                ...
                if (method_exists($that, $field_fun)) {
                    $field_value = $that->{$field_fun}($entity, $field);
                    $bool = FALSE;
                }
                if ($current_hook_alias) {
                    $alias = $current_hook_alias;
                ...
                if ($bool) {
                    ...
                    if(isset($options['#help_enable']) && $options['#help_enable']){
        ...
        if ($bool) {
            ...
            try {
                ...
                foreach ($comments as $comment) {
                    $result[] = $this->entity_parser_load($comment);   
                }
