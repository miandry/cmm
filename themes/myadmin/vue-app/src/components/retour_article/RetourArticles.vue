<template>
    <div>
        <PageLoader v-if="loader" />
        <!-- Recherche et Filtres -->
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-8">
            <div
                class="flex flex-col xl:flex-row lg:items-center space-y-4 md:space-y-0 lg:space-x-4 gap-2 md:items-end">
                <!-- Barre de Recherche -->
                <div class="w-full relative"> <!-- xl:w-2/5 -->
                    <input type="text" placeholder="Rechercher par article" v-model="searchKeyWord"
                        @input="handleSearch" @focus="showList = true" @blur="showList = false"
                        class="w-full px-4 py-2.5 pl-12 pr-4 text-gray-900 bg-gray-50 border border-gray-300 !rounded-button focus:ring-2 focus:ring-primary focus:border-primary outline-none">
                    <div
                        class="absolute left-4 top-1/2 transform -translate-y-1/2 w-5 h-5 flex items-center justify-center">
                        <i class="ri-search-line text-gray-400"></i>
                    </div>

                    <div v-if="showList" @mousedown.prevent
                        class="max-h-48 overflow-y-auto border border-gray-300 !rounded-button bg-white absolute right-0 left-0 z-50">

                        <!-- Loader -->
                        <div v-if="loading" class="flex flex-col items-center justify-center py-6">
                            <div class="w-8 h-8 border-4 border-gray-300 border-t-blue-600 rounded-full animate-spin">
                            </div>
                            <p class="text-center text-xs text-gray-600 mt-2">Chargement...</p>
                        </div>
                        <div v-else-if="articleStore.articles.rows.length" class="divide-y divide-gray-100">
                            <div v-for="article in articleStore.articles.rows" :key="article.nid"
                                :class="[
                                    'flex items-center space-x-3 px-3 py-2 hover:bg-gray-50 cursor-pointer customer-item border-t-0']"
                                @click="selectArticle(article.nid, article.title)">
                                <div class="flex-1">
                                    <p class="text-sm font-medium text-gray-900">{{ article.title }}</p>
                                </div>
                            </div>
                        </div>
                        <div v-else>
                            <h3 class="text-center text-gray-400 py-2">Aucun article trouvé avec ce mot-clé</h3>
                        </div>
                    </div>

                </div>
                <!-- Filtre Date -->
                <div class="w-full xl:w-1/5">
                    <div class="relative w-full">
                        <input type="date" placeholder="Date" v-model="selectedDate"
                            class="w-full px-4 py-2.5 text-gray-900 bg-gray-50 border border-gray-300 !rounded-button focus:ring-2 focus:ring-primary focus:border-primary outline-none">
                    </div>
                </div>
            </div>
        </div>
        <!-- Tableau Principal -->
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden" id="retourArticlesTableau">
            <div class="overflow-x-auto">
                <table class="w-full">
                    <thead class="bg-gray-50 border-b border-gray-200">
                        <tr>
                            <th class="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                Article</th>
                            <th class="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                Quantité</th>
                            <th class="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                Description</th>
                            <th class="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                Date</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-200">
                        <tr class="hover:bg-gray-50" v-for="retour in retourArticleStore.retourArticles.rows" :key="retour.nid">
                            <td class="px-6 py-4 text-left">
                                <div class="flex items-center space-x-3">
                                    <div class="w-10 h-10 bg-orange-100 rounded-lg flex items-center justify-center">
                                        <i class="ri-arrow-go-back-line text-orange-600"></i>
                                    </div>
                                    <div class="flex-1">
                                        <div class="font-medium text-gray-900 text-xs">{{ retour.field_article?.title || '-' }}
                                        </div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4 text-sm whitespace-nowrap text-left">
                                {{ retour.field_quantite || 0 }}
                            </td>
                            <td class="px-6 py-4 text-sm">
                                {{ retour.field_description || '-' }}
                            </td>
                            <td class="px-6 py-4 text-sm whitespace-nowrap text-left">
                                <span class="text-blue-500">{{ formatDate(retour.field_date, null, 'short') }}</span>
                            </td>
                        </tr>
                    </tbody>
                </table>
            </div>
            <!-- Pagination -->
            <div v-if="totalPages > 1" class="px-6 py-4 border-t border-gray-200 flex items-center justify-center">
                <div class="flex items-center space-x-2">
                    <!-- Previous -->
                    <button @click="previousPage" :disabled="currentPage === 1"
                        class="px-3 py-2 text-gray-400 hover:text-gray-600 disabled:opacity-50">
                        <i class="ri-arrow-left-s-line"></i>
                    </button>

                    <!-- Pages -->
                    <button v-for="page in visiblePages" :key="page" @click="goToPage(page)"
                        class="px-3 py-2 rounded-md transition-colors" :class="page === currentPage
                            ? 'bg-primary text-white'
                            : 'text-gray-600 hover:text-gray-900'">
                        {{ page }}
                    </button>

                    <!-- Next -->
                    <button @click="nextPage" :disabled="currentPage === totalPages"
                        class="px-3 py-2 text-gray-400 hover:text-gray-600 disabled:opacity-50">
                        <i class="ri-arrow-right-s-line"></i>
                    </button>

                </div>
            </div>
        </div>

        <!-- Modal add & edit -->
        <SaveRetourArticle class="fixed inset-0 flex bg-black bg-opacity-50 items-center justify-center z-50" v-if="openModal"
            @close="closeRetourModal" :articles="articleStore.articles.rows" @addRetourArticle="addRetourArticle"
            :retourArticle="selectedRetourArticle" />
    </div>
</template>

<script>
import { computed, nextTick, onMounted, ref, watch } from 'vue';
import { useRetourArticleStore, useArticleStore } from '../../stores/index.js';
import PageLoader from '../PageLoader.vue';
import SaveRetourArticle from './SaveRetourArticle.vue';
import { formatDate } from '../../utils/formateDate.js';
import { debounce } from 'lodash';

export default {
    name: "RetourArticles",
    components: {
        PageLoader,
        SaveRetourArticle,
    },
    props: {
        openModal: {
            type: Boolean,
            required: true,
        },
        selectedRetourArticle: {
            type: Object,
            default: null
        }
    },
    emits: ['openModal', 'close'],
    setup(props, { emit }) {
        const retourArticleStore = useRetourArticleStore();
        const articleStore = useArticleStore();
        const perPage = 15;
        const currentPage = ref(1);
        const totalPages = computed(() => Math.ceil(retourArticleStore.retourArticles.total / perPage));
        const loader = ref(false);
        const isFirstLoad = ref(true);
        const searchKeyWord = ref('');
        const selectedDate = ref('');
        const showList = ref(false);
        const loading = ref(false);

        // Paramètres dynamiques de la requête
        const queryOptions = ref({
            fields: [
                'nid',
                'title',
                'field_article',
                'field_quantite',
                'field_description',
                'field_date',
            ],
            sort: { val: 'nid', op: 'desc' },
            filters: {
                status: {
                    val: 1,
                    op: "="
                }
            },
            pager: 0,
            offset: 15
        })

        const queryOptionsArticle = ref({
            fields: [
                'nid',
                'title'
            ],
            sort: { val: 'nid', op: 'desc' },
            filters: {
                status: {
                    val: 1,
                    op: "="
                }
            },
            pager: 0,
            offset: 10
        })

        const fetchArticles = async (append = false) => {
            try {
                await articleStore.fetchArticles(queryOptionsArticle.value, append)
            } catch (error) {
                console.error("une erreur c'est produit lors du chargement des articles")
            }
        }

        const fetchRetourArticles = async (append = false) => {
            try {
                loader.value = true
                await retourArticleStore.fetchRetourArticles(queryOptions.value, append)
            } catch (error) {
                console.error("une erreur c'est produit lors du chargement des données")
            } finally {
                loader.value = false
                if (isFirstLoad.value) {
                    isFirstLoad.value = false
                    return
                }
                await nextTick();

                const el = document.getElementById('retourArticlesTableau');
                el?.scrollIntoView({
                    behavior: 'smooth',
                    block: 'start'
                });
            }
        }

        onMounted(async () => {
            await fetchRetourArticles(false)
        })

        // Pages visibles (3 pages max)
        const visiblePages = computed(() => {
            const pages = [];
            const total = totalPages.value;
            const current = currentPage.value;

            if (total <= 3) {
                for (let i = 1; i <= total; i++) pages.push(i);
            } else {
                if (current === 1) pages.push(1, 2, 3);
                else if (current === total) pages.push(total - 2, total - 1, total);
                else pages.push(current - 1, current, current + 1);
            }

            return pages;
        });

        // Actions
        const goToPage = page => {
            if (page >= 1 && page <= totalPages.value) {
                currentPage.value = page;
                onPagination(currentPage.value)
            }
        };

        const nextPage = () => {
            if (currentPage.value < totalPages.value) {
                currentPage.value++
                onPagination(currentPage.value)
            };
        };

        const previousPage = () => {
            if (currentPage.value > 1) {
                currentPage.value--
                onPagination(currentPage.value)
            };
        };

        const onPagination = async (value) => {
            queryOptions.value.pager = value - 1;
            await fetchRetourArticles(false);
        }

        const closeRetourModal = () => {
            emit('close')
        }

        const handleSearch = () => {
            showList.value = true;
            loading.value = true;
            debouncedFetch();
        }

        const debouncedFetch = debounce(async () => {
            if (searchKeyWord.value == "") {
                showList.value = false;
                selectArticle('', '');
                return;
            }
            updateFilter(queryOptionsArticle.value, 'title', searchKeyWord.value, 'CONTAINS')
            await fetchArticles(false);
            loading.value = false;
        }, 600);

        const selectArticle = async (nid, name) => {
            searchKeyWord.value = name;
            updateFilter(queryOptions.value, 'field_article', nid, '=')
            await fetchRetourArticles(false)
            showList.value = false;
        }

        // Fonction générique pour mettre à jour les filtres
        const updateFilter = (queryOptionsRef, key, value, op = '=') => {
            if (
                value === null ||
                value === undefined ||
                value === '' ||
                (Array.isArray(value) && value.length === 0)
            ) {
                delete queryOptionsRef.filters[key];
                return;
            }

            queryOptionsRef.filters[key] = {
                val: value,
                op
            };
        };

        const applyDateFilter = async () => {
            const date = selectedDate.value;

            if (!date) {
                if (queryOptions.value.filters.field_date) {
                    delete queryOptions.value.filters.field_date;
                    queryOptions.value.pager = 0;
                    await fetchRetourArticles(false);
                }
                return;
            }

            updateFilter(queryOptions.value, 'field_date', date, '=');
            queryOptions.value.pager = 0;
            await fetchRetourArticles(false);
        };

        const addRetourArticle = async () => {
            searchKeyWord.value = "";
            selectedDate.value = "";
            updateFilter(queryOptions.value, 'field_article', searchKeyWord.value, 'CONTAINS')
            updateFilter(queryOptions.value, 'field_date', selectedDate.value)
            await fetchRetourArticles(false);
        }

        watch(selectedDate, applyDateFilter);

        return {
            retourArticleStore,
            articleStore,
            goToPage,
            nextPage,
            previousPage,
            visiblePages,
            totalPages,
            currentPage,
            loader,
            loading,
            addRetourArticle,
            formatDate,
            closeRetourModal,
            searchKeyWord,
            handleSearch,
            selectedDate,
            selectArticle,
            showList
        }
    }
}
</script>

<style></style>
