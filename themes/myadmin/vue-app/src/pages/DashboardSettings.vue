  ...
  <div class="max-w-3xl mx-auto p-4 md:p-6">
    <div class="mb-6">
      ...
      <p class="text-sm text-gray-500 mt-1">
        Active ou désactive les éléments affichés sur la page Dashboard pour tous les utilisateurs.
      </p>
    </div>
...
import { getDashboardSettings, saveDashboardSettings } from "../services/dashboardSettings.js";
  ...
  setup() {
    ...
    const load = async () => {
      ...
      try {
        ...
        toast.error("Impossible de charger la configuration du tableau de bord.");
      } finally {
        loading.value = false;
      }
    };
    ...
    const save = async () => {
      ...
      try {
        const response = await saveDashboardSettings(disabled);
        if (response.data?.status) {
          localItems.value = (response.data.items || []).map((item) => ({ ...item }));
          dashboardStore.setDisabledKeys(response.data.disabled || disabled);
          toast.success(response.data.message || "Tableau de bord enregistré.");
        } else {
